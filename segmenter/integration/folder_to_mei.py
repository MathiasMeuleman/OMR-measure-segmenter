...
def run(sheet_name):
    ...
    page_path = Path(r'../../tmp/test')
    image_paths = sorted([str(p.resolve()) for p in page_path.iterdir() if p.is_file()], key = lambda x : int(os.path.basename(x).split('-')[1].split('.')[0]))
    ...
    mei_path = Path(r'../../data/Beethoven_Sextet')
