...
from staff_finder.stafffinder import StaffFinder as _virtual_StaffFinder

from staff_finder.stafffinder import StafflineSkeleton
import staff_finder.plugins.line_tracking
...
"""
    ...
    def __init__(self, img, staffline_height=0, staffspace_height=0):
        ...
        _virtual_StaffFinder.__init__(self, img, staffline_height, staffspace_height)
...
"""
        ...
        skeleton_list = self.image.get_staff_skeleton_list(self.staffline_height, window * self.staffspace_height, blackness)
        ...
        for g in groups:
            ...
            # TODO: Check if groups have been separated!
        ...
        while i < len(groups) and j < len(groups) - 1:
            ...
            g = groups[i]
            h = groups[j]
            if g.col_end >= h.col_start and ((h.row_start < g.row_start < h.row_end) or (h.row_start < g.row_end < h.row_end)):
        ...
        for g in rem_groups:
            if g in groups:
                groups.remove(g)
        if debug > 0 and len(rem_groups) > 0:
            print("   %i removed, %i staffs left." % (len(rem_groups), len(groups)))
        ...
        if debug > 1:
            ...
            staff_skeleton_short = self.image.skeleton_list_to_image(too_short_skeletons)
            ...
            for c in connections:
                rgb.draw_line((c.col, c.row_start), (c.col, c.row_end), RGBPixel(0, 0, 255))  # blue
    ...
    def melt(self, other):
        ...
        for col in range(min(self.col_start, other.col_start),
            ...
            if in_self and not in_other:
                points.append(CorePoint(col, self.skeleton[1][col - self.col_start]))

            elif in_other and not in_self:
                points.append(CorePoint(col, other.skeleton[1][col - other.col_start]))
